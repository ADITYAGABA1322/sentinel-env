...
def save_memory(session_id: str, event: str, data: dict[str, Any] | None = None) -> None:
    _memory_store.add(session_id, event, data or {})
...
# ---------------------------------------------------------------------------
class MemoryStore:
    def __init__(self, ttl_seconds: int = 3600, max_events: int = 100):
        self._data: dict[str, list[dict[str, Any]]] = {}
        self._lock = RLock()
        self._ttl = ttl_seconds
        self._max_events = max_events

    def add(self, session_id: str, event: str, data: dict[str, Any]):
        with self._lock:
            ...
            if len(self._data[session_id]) > self._max_events:
                self._data[session_id].pop(0)

    def get(self, session_id: str):
        now = time.time()
        with self._lock:
            events = self._data.get(session_id, [])

            
            filtered = [e for e in events if now - e["ts"] < self._ttl]
            self._data[session_id] = filtered
...
def health():
    return {
        ...
        "memory_sessions": len(_memory_store._data),
    }
...
def step(req: StepRequest, session_id: str = Query(...)):
    ...
    try:
        ...
        save_memory(session_id, "step", {
            "action": req.action_type,
            "specialist": req.specialist_id,
            "reward": result.get("reward"),
            "trust_snapshot": result["observation"]["trust_snapshot"],
    ...
    if result["done"]:
        ...
        _memory_store.clear(session_id)
    else:
        result["info"]["orchestrator_prompt"] = build_orchestrator_prompt(result["observation"])
...
def state(session_id: str = Query(...)):
    env = _get_env(session_id)
    result = env.state(session_id=session_id)
    memory = _memory_store.get(session_id)
    result["memory"] = memory
    result["memory_summary"] = [
       f"{e['event']} → reward: {e['data'].get('reward')}"
        for e in memory[-5:]
    ]
    return result

@app.get("/memory")
def memory(session_id: str = Query(...)):
    events = _memory_store.get(session_id)
...
def debug_session(session_id: str = Query(...)):
    env = _get_env(session_id)

    return {
        "state": env.state(session_id),
        "memory": _memory_store.get(session_id),
        "session_store_stats": _sessions.stats(),
        "env_snapshot": env.stream_snapshot() if hasattr(env, "stream_snapshot") else None
    }
...
def mcp(body: dict[str, Any]):
    ...
    if method == "reset":
        ...
        session_id = result["info"]["session_id"]
        save_memory(session_id, "reset", {"params": params})
        _sessions.set(session_id, env)
        result["info"]["mission"] = mission_for_task(result["observation"]["task_type"])
        result["info"]["orchestrator_prompt"] = build_orchestrator_prompt(result["observation"])
        memory = get_memory(session_id)
        if memory is not None:
            result["memory"] = memory[:3] if isinstance(memory, list) else memory
        ...
        save_memory(session_id, "step", {
            ...
            "specialist": params.get("specialist_id"),
            "reward": result.get("reward"),
        ...
        result = _get_env(session_id).state(session_id)
        memory = get_memory(session_id)
        if memory is not None:
            result["memory"] = memory[:3] if isinstance(memory, list) else memory
        return {"result": result}
