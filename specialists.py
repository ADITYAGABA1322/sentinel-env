...
class SpecialistPool:
    ...
    def public_ground_truth_reliability(self, internal_reliability: dict[str, float]) -> dict[str, float]:
        """
        Map internal (behavior-type) reliabilities to the *current episode's*
        public slot labels, based on the shuffled profile.

        `internal_reliability` is keyed by internal ids ("S0".."S4") corresponding
        to the fixed behavior implementations. The returned dict is keyed by
        public slot ids ("S0".."S4") used by the agent this episode.
        """
        out: dict[str, float] = {}
        for pub, internal in self._profile.items():
            out[pub] = float(internal_reliability.get(internal, 0.5))
        return out